 ...
 * @since: 1.0.0
 */
open class SpaceItemActivityDecoration(
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (layoutMgr != null) {
            if (layoutMgr is GridLayoutManager) {
                ...
                // 瀑布流状态
                setItemOffsetsByStaggerGrid(view, outRect, layoutMgr, position)
            }
        }
    }
    ...
    private fun setItemOffsetsByLinear(outRect: Rect, layoutMgr: LinearLayoutManager, position: Int, childCount: Int) {
        val s = space ?: 0
        ...
        if (layoutMgr.orientation == LinearLayoutManager.VERTICAL) {
            ...
            outRect.left = if (position == 0) space ?: 0 else left
            outRect.right = right
        }
    }

    private fun setItemOffsetsByStaggerGrid(view: View, outRect: Rect, layoutMgr: StaggeredGridLayoutManager, position: Int) {
        val sc = layoutMgr.spanCount
        val h = spaceH ?: (space ?: 0) // 列与列之间的间距
        val v = spaceV ?: (space ?: 0) // 行与行之间的间距
